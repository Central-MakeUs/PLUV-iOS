...
class RecentViewController: UIViewController {
    ...
    private let recentTableView = UITableView().then {
    ...
    private func setData() {
        self.recentTableView.rx.setDelegate(self)
            .disposed(by: disposeBag)
        
        /// CollectionView에 들어갈 Cell에 정보 제공
        self.viewModel.meItems
            .observe(on: MainScheduler.instance)
            .bind(to: self.recentTableView.rx.items(cellIdentifier: RecentTableViewCell.identifier, cellType: RecentTableViewCell.self)) { index, item, cell in
                cell.prepare(me: item)
            }
            .disposed(by: disposeBag)
        
        /// 아이템 선택 시 다음으로 넘어갈 VC에 정보 제공
        self.recentTableView.rx.modelSelected(Me.self)
